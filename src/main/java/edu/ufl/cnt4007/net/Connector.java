...
/**
 * Connector is responsible for establishing outgoing connections to peers with lower peer IDs to
 * avoid duplicate connections.
 *
 * <p>For each successful connection, it spawns a new ConnectionHandler to manage communication.
 */
    ...
    for (PeerInfo peerInfo : peerList) {
      ...
      if (peerInfo.getPeerId() < peerContext.getPeerId()) {
        try {
          ...
          System.err.println(
              "[ERROR] Failed to connect to peer " + peerInfo.getPeerId() + ": " + e.getMessage());
        }
      }
    }
  }

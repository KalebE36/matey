...
public class PeerClient implements Runnable {
  ...
  public PeerClient(int myPeerId, PeerConfig peerConfig, PeerProcess peerProcess) {
    ...
    // Peerprocess needed for registering servers
  ...
  public void run() {
    System.out.println(
        "[DEBUG] Peer Client thread started. Will attempt to connect to preceding peers.");
          ...
          if ((!peerProcess.doesClientExist(neighborInfo.peerId))
            ...
            try {
              ...
              ServerHandler clientHandler =
                  new ServerHandler(neighborInfo.peerId, clientSocket, this);

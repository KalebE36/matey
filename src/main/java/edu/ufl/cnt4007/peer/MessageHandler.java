...
public class MessageHandler {
    ...
    public void onMessage(int remotePeerId, Message message) {
        switch (message.type()) {
            ...
            default ->
                System.err.println("[ERROR] Unknown message type from peer " + remotePeerId + ": " + message.type());
        }
    }

    /**
     * Helper: Sends a message to a specific peer, handling ConnectionHandler
     * retrieval and network errors.
     */
    private void safelySend(int remotePeerId, Message message, String actionDescription) {
        ...
        try {
            ...
            System.out
                    .println("[INFO] Sent " + message.type() + " (" + actionDescription + ") to peer " + remotePeerId);
        } catch (Exception e) {
            System.err.println("[ERROR] Failed to send " + message.type() + " (" + actionDescription + ") to peer "
                    + remotePeerId + ": " + e.getMessage());
        }
    }

    /**
     * Helper: Sends INTERESTED or NOT_INTERESTED message based on current state
     * check.
     */
    ...
    /**
     * Helper: Finds the next missing piece and sends a REQUEST message if
     * available.
     * Sends NOT_INTERESTED if no missing pieces are found.
     */
    private void attemptRequestNextPiece(int remotePeerId) {
        ...
        if (nextPieceIndex == -1) {
            System.out.println(
                    "[INFO] No missing pieces to request from peer " + remotePeerId + ". Sending NOT_INTERESTED.");
            // We are not interested anymore
            safelySend(remotePeerId, new Message(MessageType.NOT_INTERESTED, null),
    ...
    /**
     * Helper: Broadcasts a HAVE message for the given piece index to all connected
     * peers.
     */
    private void broadcastHave(int pieceIndex) {
        Message haveMessage = new Message(MessageType.HAVE,
        ...
        for (Map.Entry<Integer, ConnectionHandler> con : peerState.getConnections().entrySet()) {
            ...
            try {
                ...
                System.err.println("[ERROR] Failed to send HAVE to peer " + remotePeerId + ": " + e.getMessage());
            }
        }
    }
    ...
    private void broadcastNotInterested() {
        ...
        for (Map.Entry<Integer, ConnectionHandler> con : peerState.getConnections().entrySet()) {
            ...
            try {
                ...
                System.err.println(
                        "[ERROR] Failed to send NOT_INTERESTED to peer " + remotePeerId + ": " + e.getMessage());
            }
        }
    }
    ...
    private void handleRequestMessage(int remotePeerId, Message message) {
        ...
        byte[] payload = ByteBuffer.allocate(4 + (int) pieceSize)
                .putInt(pieceIndex)
                .array();

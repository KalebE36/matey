...
/**
 * Represents a bitfield for tracking piece availability in a P2P file sharing system.
 *
 * <p>Each bit in the bitfield indicates whether a specific piece of the file is present (1) or
 * absent (0). Internally, the bitfield is stored as a {@code byte[]} array, where each byte
 * contains 8 bits.
 *
 * <p>Example interpretation:
 *
 * <ul>
 *   <li>First byte: 0b10101010 → pieces 0, 2, 4, 6 are present
 *   <li>Second byte: 0b00001111 → pieces 8, 9, 10, 11 are present
 * </ul>
 *
 * <p>Bitfields can be compared using bitwise operations to determine which pieces are available in
 * one bitfield but not the other.
 *
 * <p>Example usage:
 *
 * <pre>{@code
...
public class Bitfield {
  ...
  /**
   * Set all pieces as present in the bitfield. Mainly used when initializing a seed peer.
   *
